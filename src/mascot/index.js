...
const cubismCode = fs.readFileSync(VENDOR_CUBISM_CORE_PATH, 'utf8');
const script = document.createElement('script');
script.textContent = cubismCode;
...
const DEFAULT_MODEL_PATH = normalizeMascotModelPath(store.get('mascotModelPath', DEFAULT_MASCOT_MODEL_PATH));
...
async function loadMascotModel(modelPath = DEFAULT_MODEL_PATH) {
  try {
    const resolved = normalizeMascotModelPath(modelPath || DEFAULT_MODEL_PATH);
    const modelFull = path.join(APP_ROOT, resolved);
    console.log('[Mascot] Loading model:', modelFull);
    
    if (!fs.existsSync(modelFull)) {
      console.error('[Mascot] Model file not found:', modelFull);
      return;
    }

    const modelUrl = pathToFileURL(modelFull).href;
    ...
    console.error('[Mascot] Model load error:', err);
  }
}
