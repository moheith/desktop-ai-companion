...
async function loadChatModel(modelPath){
  try {
    if(!chatPixiApp){
      new Function(fs.readFileSync(VENDOR_CUBISM_CORE_PATH,'utf8'))();
    ...
    const resolved=normalizeMascotModelPath(modelPath||cfg.mascotModelPath||DEFAULT_MASCOT_MODEL_PATH);
    const modelUrl=pathToFileURL(path.join(APP_ROOT,resolved)).href;
    const nextModel=await Live2D.from(modelUrl);
    if(chatModel){
...
ipcRenderer.on('init-config',(_, s)=>{
  Object.assign(cfg,s);
  cfg.mascotModelPath=normalizeMascotModelPath(cfg.mascotModelPath);
...
function discoverMascotModels(){
  const root=path.join(APP_ROOT,'model');
  ...
  const walk=dir=>{
    for(const entry of fs.readdirSync(dir,{withFileTypes:true})){
      ...
      if(entry.isDirectory()) walk(full);
        ...
        const rel=path.relative(APP_ROOT,full).replace(/\\/g,'/');
...
function previewMascotModel(modelPath){
  const selected = normalizeMascotModelPath(String(modelPath||'').trim());
  if(!selected) return;
...
function getBundledWhisperPaths(){
  ...
  if(typeof process!=='undefined'){
    ...
    roots.push(path.join(APP_ROOT,'mic','whisper-bin-x64','Release'));
...
document.getElementById('mascot-save-btn').onclick=()=>{
  ...
  cfg.mascotModelPath=normalizeMascotModelPath(val('mascot-model-select')||cfg.mascotModelPath);
